...
class ItemizedInspectionReport(models.Model):
	# score_report pulls from Scores object which outputs the columns for the tables.
	score_report = models.ForeignKey('Scores', on_delete=models.SET_NULL, null=True)
...
class Scores(models.Model):	
	# maybe change house to uninspectable house
	house = models.ForeignKey('houses.House', on_delete=models.SET_NULL, null=True)
	date = models.DateField(default=date.today, blank=True)
	ric = models.ForeignKey('accounts.Trainee', on_delete=models.SET_NULL, null=True)
	inspectors = models.ManyToManyField('Inspectors', related_name='inspectorgroup', blank=True)
	score = models.DecimalField(max_digits=4, decimal_places=2, default=0.00, null=True)
	notes = models.TextField(null=True, blank=True)
	uninspectable = models.BooleanField(default=False)
	uninspectable_reason = models.TextField(null=True, blank=True)
	def __str__(self):
		return '%s' % (self.house)
...
class Inspectors(models.Model):
	...
	def __str__(self):
		return '%s' % (self.full_name)#(self.first_name, self.last_name)

class InspectableHouses(models.Model):
	residence = models.ForeignKey('houses.House', on_delete=models.SET_NULL, null=True)
	residence_type = models.CharField(max_length=10, default=None)
	uninspectable = models.BooleanField(default=False)
	def __str__(self):
